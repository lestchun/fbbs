...
var ___baseNamespaceName = "CproNamespace"; (function() {
    ...
    if (!b && (top.location.href.indexOf("union.baidu.com") > 0 || top.location.href.indexOf("unionqa.baidu.com") > 0)) {
        b = true
    }
    var d = function(h, j, i) {
        ...
        h.registerMethod = function(o, k) {
            ...
            if (o && o.instances && o.instances.length && o.instances.length > 0) {
                for (var m = 0,
                n = o.instances.length; m < n; m++) {
                    r = o.instances[m];
                ...
                if (n) {
                    try {
                        if (this.readyFuncArray && this.readyFuncArray.length) {
                            for (var k = 0,
                            m = this.readyFuncArray.length; k < m; k++) {
...
})(window[___baseNamespaceName]); (function(b) {
    var c = {
        ...
        getSlotDataFromUnion: function(j, n, f) {
            if (!j || !n || !window[n]) {
                return null
            }
            ...
            var k = "http://pos.baidu.com/ecom?di={slotId}&tm=BAIDU_CPRO_SETJSONADSLOT&fn={callbackName}&word={word}&winh={winh}&winw={winw}&baidu_id={baiduId}";
            var h = this.U.getCookieRaw("BAIDUID") || "";
            var p = this.U.template(k, {
                slotId: j,
                callbackName: n,
                baiduId: h,
                winh: j.indexOf(",") > 0 ? 1 : this.U.getClientHeight(window),
                winw: j.indexOf(",") > 0 ? 1 : this.U.getClientWidth(window),
                word: encodeURIComponent(g(this))
            });
            var l = i(this);
            var e = j;
            var o;
            var d;
            if (l && e == l.sid) {
                p = p.split(/&(?=baidu_id)/).join("&mid=" + l.mid + "&sid=" + l.vc + "&")
            }
            if (/u\d+/.test(j)) {
                if (f) {
                    ...
                    document.write('<script type="text/javascript" charset="utf-8" src="' + p + '"><\/script>')
                }
            } else {
                window[n](null, {
                    _html: j
            ...
            function i(w) {
                if (w.U.isInCrossDomainIframe()) {
                    var v = window.location.search.slice(1)
                } else {
                    var v = top.location.search.slice(1)
                }
                var q = w.U.getUrlQueryValue(v, "baidu_clb_preview_sid");
                var s = w.U.getUrlQueryValue(v, "baidu_clb_preview_mid");
                var t = w.U.getUrlQueryValue(v, "baidu_clb_preview_vc");
                var u = +w.U.getUrlQueryValue(v, "baidu_clb_preview_ts");
                var r = +new Date;
                if (r - u <= 30 * 1000) {
                    return {
                        sid: q,
        ...
        checkAdsCounter: function(f, h, e) {
            var d = false;
            var g;
            if (e && (e.indexOf("tlink") > -1 || e.indexOf("baiduTlinkInlay") > -1)) {
                f = "linkunit"
            }
        ...
        addAdsCounter: function(e, f, d) {
            if (d && (d.indexOf("tlink") > -1 || d.indexOf("baiduTlinkInlay") > -1)) {
                e = "linkunit"
            }
    ...
    var a = {
        ...
        serialize: function(g) {
            ...
            var d, c;
            for (d in g) {
                if (d && g[d] && (typeof g[d] === "object") && g[d].isUIParam && g[d].isUIParam[g.displayType]) {
                    if (d === "pn" && !g.get(d)) {
                        continue
                    }
                    c = g.get(d);
                    ...
                    if (g.displayType == "ui" && c == "baiduCADS") {
                        continue
                    }
                    if (g[d].encode || g.displayType == "ui") {
                        c = encodeURIComponent(c)
                    }
                    if (g[d].limit) {
                        c = c.substr(0, g[d].limit)
                    }
                    e.push(d + "=" + c)
                }
            }
            if (g.get("tn") === "baiduTlinkInlay" && g.displayType === "inlay") {
                var f = this.BL.getLinkUnitMaxCount(g);
                if (g.get("adn") > f.VerticalCount * f.HorizontalCount) {
                    e.push("hn=" + f.HorizontalCount);
                    e.push("wn=" + f.VerticalCount)
                } else {
                    if (parseInt(g.get("adn")) <= f.VerticalCount) {
                        e.push("hn=" + 1);
                        e.push("wn=" + parseInt(g.get("adn")))
                    } else {
                        var h = Math.ceil(parseInt(g.get("adn")) / f.VerticalCount);
                        e.push("hn=" + h);
                        e.push("wn=" + f.VerticalCount)
                    }
                }
        ...
        titFF: {
            ...
            get: function() {
                return encodeURIComponent(this["titFF"]._value)
            },
            set: "default"
        },
        titFS: {
            slotParamName: "cpro_titFS",
            modifier: "dynamic",
            defaultValue: {
                inlay: 12
            },
    ...
    var a = {
        ...
        windowOnUnload: function(m) {
            try {
                ...
                if (document.domain.toLowerCase().indexOf("autohome.com.cn") > -1 || document.domain.toLowerCase().indexOf("sina.com.cn") > -1 || document.domain.toLowerCase().indexOf("xcar.com.cn") > -1) {
                    return
                }
    ...
    b.registerClass(a)
})(window[___baseNamespaceName]); (function(l) {
    var l = l;
    ...
    var n = function(v, w, i) {
        ...
        if (i) {
            u.onload = i;
            u.onreadystatechange = function() { (u.readyState === "loaded" || u.readyState === "complete") && i()
            }
        }
        u.src = w;
        v.body.insertBefore(u, v.body.firstChild)
    };
    var e = window[t] = function(I, R) {
        var G;
        for (G in I) {
        ...
        if (w._isMlt && w._isMlt == 3) {
            ...
            window.BAIDU_CLB_SLOTS_MAP[G] = w;
            if (window.BAIDU_DAN_showAdArray) {
                window.BAIDU_DAN_showAd ? window.BAIDU_DAN_showAd(G, u) : window.BAIDU_DAN_showAdArray.push([G, u])
            } else {
                window.BAIDU_DAN_showAdArray = window.BAIDU_DAN_showAdArray || [];
                window.BAIDU_DAN_showAdArray.push([G, u]);
                n(document, "http://cbjs.baidu.com/js/dn.js",
                function() {
                    while (window.BAIDU_DAN_showAdArray.length) {
                        var U = window.BAIDU_DAN_showAdArray.shift();
                        window.BAIDU_DAN_showAd(U[0], U[1])
                    }
                })
            }
            return
        ...
        var N = 0;
        s.randomArray.push(Math.random() * 1000000);
        S = s.parseSlotDataFromUnion(I);
        H = s.getSlotDataFromUserOpenApi();
        if (H) {
            N = 1;
            z = d.extend(S, H)
        } else {
        ...
        F = s.Param.getSlot2UIMapping(T);
        T = s.Param.mergeSlot2UI(T, z, F);
        T.set("js", a);
        if (N) {
            T.set("c01", 1)
        }
        var i = R || "cpro_" + T.get("tu");
        var A = d.g(i, window);
        var D = A ? true: false;
        var Q = T.get("rsi0");
        var E = T.get("rsi1");
        var O = T.get("at");
        var M = T.at._value;
        var L = T.get("tn").toString();
        if (s.isPreview(Q, E, k, O)) {
            if (D) {
                ...
                P.setAttribute("src", "http://cpro.baidu.com/cpro/ui/inlayPreview.js");
                var K = document.getElementsByTagName("script")[0];
                K.parentNode.insertBefore(P, K)
            } else {
                window.cpro_isSyncPreview = true;
                window.cpro_template = T.get("tn").toString();
                window.cpro_w = Q.toString();
                window.cpro_h = E.toString();
            ...
            if (s.checkAdsCounter(k, window, L)) {
                return
            }
            ...
            var J = s.getAdsDomId(T.get("did"));
            var C = s.Param.serialize(T);
            var y = T.get("n").toString();
            if (y == "07095078_1_cpr" || y == "07095078_cpr" || y == "71049059_cpr") {
                C = C + "&titSU=0"
            }
            var x = {
                iframeId: J,
            ...
            if (D || R) {
                if (A) {
                    A.innerHTML = d.template(v, x)
                }
            } else {
                document.write(d.template(v, x))
            }
            var B = s.Param.snap(T);
            s.adsArray.push({
                domId: J,
                uiParamSnap: B,
                win: window,
    ...
    if (a === "cc") {
        ...
        if (a === "c") {
            ...
            if (window.cproArray) {
                var b = [];
                for (var p = 0,
                g = window.cproArray.length; p < g; p++) {
